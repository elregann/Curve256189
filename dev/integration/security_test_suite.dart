...
class Curve256189SecurityTest {
  ...
  // Section 1: Invalid Point Decode Test
  // Ensures decodePoint rejects invalid encodings:
  // - Random bytes should not decode to valid points
  // - Points that decode must be on the curve
  static void testInvalidPointDecode() {
    print('Running invalid point decode test (100000 iterations)...');
    ...
    for (int i = 0; i < 100000; i++) {
      ...
      if (P != null) {
        ...
        if (!TwistedEdwards.isOnCurve(P)) {
          print('FAIL: Point decoded but is not on the curve');
          return;
        }
      }
    }
  ...
  static void testSignatureNonMalleability() {
    ...
    if (s >= n) {
      // Signature is already non-canonical and should be rejected
      _checkMalleability('Non-canonical s (original)',
          !EdDSA.verify(message, signature, keypair['publicKey']!));
    } else {
      // Create a non-canonical s by adding n
  ...
  // Section 3: Point Encode/Decode Round-Trip Test
  // Verifies that encodePoint( decodePoint( encodePoint(P) ) ) = encodePoint(P)
  // for all valid points reachable from G.
  static void testPointEncoding() {
    print('Running point encode/decode round-trip test (10000 iterations)...');

    for (int i = 0; i < 10000; i++) {
      // Generate a random scalar in the full range [0, n)
  ...
  // Section 4: Run All Security Tests
  static void runAll() {
